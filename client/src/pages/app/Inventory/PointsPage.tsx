...
import { useGetUserScoreLogsQuery, useGetUsersQuery, useUpdateUserScoreMutation } from '../../../features/crud/users/usersApi';
...
const PointsPage: React.FC = () => {
    ...
    const { data: userScoreLogs } = useGetUserScoreLogsQuery(selectedUser?.id ?? 0);
    ...
    const handleUpdateScore = (userId: number, points: number, reason: string, details: string) => {
        updateUserScore({ id: userId, points, reason, details });
    }
    ...
    return (
        <IonPage>
                ...
                <IonItem>
                    <div style={{ display: 'flex', alignItems: 'center', width: '100%', margin: '16px' }}>
                        <div>
                            ...
                            <IonButton onClick={() => setIsModalOpen(true)}>Edit score</IonButton>
            ...
            <IonModal isOpen={isModalOpen} onDidDismiss={() => setIsModalOpen(false)}>
                <IonContent>
                    <Header title="Edit Score" type='back' onBack={() => setIsModalOpen(false)} />
                    <div style={{ padding: '20px' }}>
                        ...
                        <IonButton
                            ...
                            onClick={() => {
                                ...
                                handleUpdateScore(selectedUser!.id, pointsToAdd, reasoning!, details);
            ...
            <IonModal isOpen={isHistoryModalOpen} onDidDismiss={() => setIsHistoryModalOpen(false)}>
                <IonContent>
                    <Header title="Score History" type='back' onBack={() => setIsHistoryModalOpen(false)} />
                    <div style={{ padding: '20px' }}>
                        <IonList>
                            {userScoreLogs && userScoreLogs.length > 0 ? userScoreLogs.map(log => (
                                <IonItem key={log.id}>
                                    <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', padding: '16px 0 16px 0' }}>
                                            <IonLabel>
                                                ...
                                                <p>{new Date(log.loggedAt).toLocaleString()}</p>
                                            </IonLabel>
