...
const ModelPage: React.FC = () => {
    ...
    const [addModel, { isLoading: isUpdating }] = useAddModelMutation();
    ...
    const handleFormSubmit = async () => {
        try {
            ...
            setToast({ show: true, message: 'Error adding model. Try again.', color: 'danger' });
        }
    };
        ...
        <IonPage>
            ...
            <IonContent fullscreen>
                <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    {modelsLoading ? (
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
                            {models?.map(model => (
                                <Card key={model.id} style={{ position: 'relative' }}>
                ...
                <IonModal isOpen={formOpen} onDidDismiss={() => setFormOpen(false)}>
                    <IonHeader>
                        <IonToolbar>
                            <IonTitle>Add 3D Model</IonTitle>
                            <IonButton
                                slot="end"
                                fill="clear"
                        ...
                        <div style={{ padding: '16px' }}>
                            ...
                            <div style={{ marginTop: '20px', display: 'flex', gap: '10px' }}>
                                <IonButton 
                                    ...
                                    disabled={isUpdating || !modelForm.name.trim()}
                                >
                                    {isUpdating ? 'Adding...' : 'Add Model'}
                                </IonButton>
                                <IonButton 
                                    expand="block" 
                                    fill="outline" 
