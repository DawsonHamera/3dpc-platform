...
export const useOneSignal = () => {
    ...
    if (!ctx) throw new Error("useOneSignal must be used within OneSignalProvider");
    return ctx;
};
...
export const OneSignalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
        ...
        const initOneSignal = async () => {
            if (typeof window !== 'undefined' && 'serviceWorker' in navigator) {
                try {
                    ...
                    console.log('Service worker ready, initializing OneSignal...');
    ...
    const subscribe = useCallback(async (user: User) => {
        ...
        try {
            ...
            await OneSignal.User.addTags({ role: user.role.name, username: user.name });
            const sub = await OneSignal.User.PushSubscription.optedIn;
            setIsSubscribed(!!sub);
            return !!sub;
        } finally {
    ...
    return (
        <OneSignalContext.Provider value={{ isSubscribed, loading, subscribe, unsubscribe, checkSubscription }}>
            {children}
